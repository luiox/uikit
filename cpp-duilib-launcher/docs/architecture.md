...
- `UiBuilder`：独立构建控件树与样式。
- `ListController`：组/项目列表选择、拖拽、右键。
- `SearchController`：搜索模式与输入同步。
- `DialogManager`：分组新增/重命名对话框。
- `ContextMenuHandler`：菜单命令分发表。

以上拆分可在不改变 UI 交互的前提下逐步迁移，降低回归风险。