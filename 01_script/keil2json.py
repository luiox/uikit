...
class CompileCommandsGenerator:
    ...
    def __init__(self, path=None, absolute=False, keil_implicit=True, extra_defines=None):
        ...
        self.defines = []
        self.source_files = []
        self.extra_defines = extra_defines if extra_defines is not None else []
    ...
    def generate_entries(self, include_paths, defines, source_files):
        ...
        base_args = [
            # "-c",
            "-D__GNUC__",
        ] + [f"-D{macro}" for macro in self.extra_defines] + \
...
if __name__ == '__main__':
    ...
    generator = CompileCommandsGenerator(path=args.path, absolute=args.absolute, keil_implicit=args.keil_implicit, extra_defines=user_macros)
    generator.generate()
