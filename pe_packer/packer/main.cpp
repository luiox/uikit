/*
PE加壳工具主程序
本程序用于对PE文件（Windows可执行文件）进行加壳处理，主要流程如下：
1. 读取输入PE文件，获取其二进制内容。
2. 将PE文件的各个节数据映射到内存，模拟程序运行时的内存布局。
3. 对映射后的内存数据进行压缩（使用LZNT1算法）。
4. 构造packingRecord结构，保存原始PE的入口点和数据目录信息。
5. 构造新的PE节结构：
   - 第一个节：RWX权限，迷惑分析工具。
   - 第二个节：壳stub代码，作为新入口点。
   - 第三个节：压缩后的原始数据。
   - 第四个节：packingRecord信息。
6. 修正PE头部信息，关闭ASLR，设置新入口点。
7. 输出加壳后的PE文件。
...
// 将PE文件的各个节数据映射到内存，模拟程序运行时的内存布局
bool dumpMappedImgBin(char*buf, BYTE* &mappedImg, size_t* imgSize) {
	PIMAGE_SECTION_HEADER stectionArr = getSectionArr(buf);
	// 计算所有节的总虚拟大小（不包括头部）
	*imgSize = getNtHdr(buf)->OptionalHeader.SizeOfImage - stectionArr[0].VirtualAddress; // 从第一个节开始
...
// 用于保存原始PE的DataDirectory和入口点，方便壳运行时还原
struct packingRecord {
	IMAGE_DATA_DIRECTORY prevDataDirTable[15]; // 原始PE的15个数据目录
	DWORD orginalEntryPoint; // 原始入口点
};

// 将壳stub、压缩数据、原始信息等写入PE文件，构造加壳后的新结构
void linkBin(char* buf, BYTE* stub, size_t stubSize, BYTE* compressedImgData, size_t compressedDataSize) {
	...
	// --------- 构造第一个节（RWX内存区，迷惑分析工具） ---------
	memcpy(&(sectionArr[0].Name), "adr", 8); // 节名伪装
	...
	// --------- 构造第二个节（壳stub代码） ---------
	memcpy(&(sectionArr[1].Name), "have", 8); // 节名伪装
	sectionArr[1].Misc.VirtualSize = stubSize; // stub代码大小
	...
	memcpy((PVOID)((UINT_PTR)buf + sectionArr[1].PointerToRawData), stub, stubSize); // 写入stub代码
	getNtHdr(buf)->OptionalHeader.AddressOfEntryPoint = sectionArr[1].VirtualAddress; // 设置新入口点为stub

	// --------- 构造第三个节（压缩后的原始数据） ---------
	memcpy(&(sectionArr[2].Name), "30cm", 8); // 节名伪装
	...
	memcpy((PVOID)((UINT_PTR)buf + sectionArr[2].PointerToRawData), compressedImgData, compressedDataSize); // 写入压缩数据

	// --------- 构造第四个节（packingRecord信息） ---------
	memcpy(&(sectionArr[3].Name), ".tw\x00", 8); // 节名伪装
	...
	memcpy((PVOID)((UINT_PTR)buf + sectionArr[3].PointerToRawData), currPackingRecord, sizeof(packingRecord)); // 写入packingRecord

	// --------- 修正PE头部信息，保证加载正常 ---------
	getNtHdr(buf)->OptionalHeader.DllCharacteristics &= ~(IMAGE_DLLCHARACTERISTICS_DYNAMIC_BASE); // 关闭ASLR
	getNtHdr(buf)->FileHeader.NumberOfSections = 4; // 总节数
